...
document.addEventListener("DOMContentLoaded", function() {
    ...
    wsConnection.onopen = function() {
        ...
        wsConnection.send("SFW");
        wsConnection.send("SN");
        wsConnection.send("HIS");
        wsConnection.send("FW");
        ...
        switch (words[0]) {
            case "LIC":
                switch (words[1]) {
                    ...
                    case "WU":
                        WebUpload = true
                        alert('Web Upload Activated')
                        break;
                    case "WI":
                        WebInterface = true
                        alert('Web Interface Activated')
                        break;
                    default:
                        break;
                }
                ...
                break;
            case "SN":
                var sn = words[1].split(",")
                SerialNumber = words[1];
            ...
            case "Settings":
                ...
                LoadSettings();
                ReadLicense();
                if (Radio_Activated == false) { document.getElementById("Radio").style.display = "none" }
                initMap();
                break;
            case "Networks":
            ...
            case "PWD":
                ...
                break;
            case "CTV":
                var TideValueTime = words[1].split(",")
                CurrentTideTime = TideValueTime[1] + ',' + TideValueTime[2];
                var u = parseFloat(TideValueTime[3])
                CurrentTideValue = Number.parseFloat(u).toFixed(2);
                ...
                break;
            case "FW":
                FirmwareVersion = words[1];
                SetFirmwareVersion();
                break;
            case "ServerFirmware":
                json = JSON.parse(words[1]);
                ServerFWVersion = json.ServerFWVersion;
                ServerFWLocation = json.ServerFWLocation;
                SetFirmwareVersion();
                break;
            case "IP":
                ...
                document.getElementById("serial").innerHTML = SerialNumber
